...
def gen_rf_rank_regional(
    ...
    Args:
        loader (SpikeLoader)
        rf (ReceptiveField)
        xy_div (Tuple[int, int]): Number of blocks in x, y direction.
    ...
    """
    pos = loader.pos
    x_cuts = np.linspace(pos.x.min(), pos.x.max(), xy_div[0] + 1)
    y_cuts = np.linspace(pos.y.min(), pos.y.max(), xy_div[1] + 1)
    ...
    for i in range(len(x_cuts) - 1):
        for j in range(len(y_cuts) - 1):
            ...
            if len(idx) < 10 * n_pc:
                logging.warning(f"({x_cuts[i]:.2f}, {y_cuts[j]:.2f}) has {len(idx)} neurons.")

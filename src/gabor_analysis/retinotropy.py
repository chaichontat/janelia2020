...
for i in range(6):
    if i != 2:
        u = axs[i].scatter(f.xpos, f.ypos, s=2 * g.corr, c=g.params_fit[:, i], cmap='viridis', alpha=0.9)
        axs[i].grid(0)
        axs[i].set_title(params_name[i])
        fig.colorbar(u, ax=axs[i])

# Excluding x pos due to space limitations.
u = axs[2].scatter(f.xpos, f.ypos, s=2 * g.corr, c=g.params_fit[:, -1], cmap='viridis', alpha=0.9)
axs[2].grid(0)
...
make_hexplot(g.params_fit[:, 0], 'σ', 'Size of Gabor Fit (σ) vs y-position')
make_hexplot(g.params_fit[:, -2], 'Gabor x-pos', 'Retinotropic x-pos of Gabor fit vs physical y-pos')
